...
def collect():
    filter_keys = ['Manufacturer', 'Serial', 'Product Name', 'UUID', 'Wake-up Type']
    raw_data = {}
...
def get_nic_info():
    ...
    for line in raw_data:
        if next_ip_line:
            next_ip_line = False
            nic_name = last_mac_ddr.split()[0]
            mac_addr = last_mac_ddr.split("HWaddr")[1].strip()
...
def get_disk_info():
    ...
    :return:
    '''
    raw_data = subprocess.Popen("sudo hdparm -i /dev/ada | grep Model", stdout=subprocess.PIPE, shell=True)
