...
class SecurityDevice(models.Model):
    ...
    sub_asset_type = models.SmallIntegerField(choices=sub_asset_type_choice, verbose_name="安全设备类型")
...
class StorageDevice(models.Model):
    ...
    def __str__(self):
        return '%s--%s__%s <sn:%s>' % (self.asset.name, self.get_sub_asset_type_display(), self.model, self.asset.sn)
...
class Contract(models.Model):
    ...
    sn = models.CharField('合同', max_length=128, unique=True)
...
class EventLog(models.Model):
    ...
    date = models.DateTimeField('时间事件', auto_now_add=True)
...
class NewAssetApprovalZone(models.Model):
    ...
    os_disrtibution = models.CharField("发行商", max_length=64, blank=True, null=True)
    ...
    class Meta:
        app_label = "CMDB"
        verbose_name = "新上新待批准资产"
        verbose_name_plural = "新上新待批准资产"
        ordering = ['-c_time']
