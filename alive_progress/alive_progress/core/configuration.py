    ...
    def _input(x):
        if isinstance(x, FunctionType):
            if x.__code__.co_name == 'inner_factory' \
                ...
                return getattr(name_lookup[x])

    func_file, _ = os.path.splitext(func_lookup.__file__)
    getattr = (lambda x: x) if name_index is None else lambda x:x[name_index]
    return _input
...
CONFIG_VARS = dict(
    ...
    spinners = _style_input_factory(SPINNERS, spinners, 0),
...
def create_config():
    ...
    def _parse(theme, options):
        '''
        Validate and convert aome configuration options.
        :param theme:
        :param options:
        :return:
        '''
